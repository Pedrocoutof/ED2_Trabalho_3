...
void NoArvoreB::insereVazio(InfoNo k, int * comparacaoInsercao)
{
    int index = n - 1;
    ...
    *comparacaoInsercao++;
    ...
    else
    {
        ...
        if (no[index + 1]->n == m - 1)
        {
            cisao(index + 1, no[index + 1], comparacaoInsercao++);

            if (chaves[index + 1].review_id < k.review_id)
                index++;
        }
        no[index + 1]->insereVazio(k, comparacaoInsercao);
    }
}

void NoArvoreB::cisao(int index, NoArvoreB *overflow, int *comparacaoInsercao)
{
